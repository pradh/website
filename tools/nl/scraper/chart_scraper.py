...
flags.DEFINE_string('queryset', 'queryset.csv',
                    'List of queries to load charts for')
flags.DEFINE_string('output_path', '', 'Output file name')
...
def _to_svg(svg):
  ...
  with open(str(_idx) + '_image.svg', 'wb') as f:
    print(f'Writing to {_idx}_image.svg')
    f.write(svg_bytes)
    _idx += 1
...
def main(_):
  assert FLAGS.queryset and FLAGS.output_path
  ...
  query2charts = {}
  with open(FLAGS.queryset) as fi:
    for q in fi.readlines():
      query2charts[q.strip()] = scrape(q, driver)

  with open(FLAGS.output_path, 'w', encoding='utf-8') as fo:
    json.dump(query2charts, fo, indent=2, ensure_ascii=False)
